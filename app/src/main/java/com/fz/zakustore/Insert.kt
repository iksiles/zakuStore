...
class Insert : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnSave.setOnClickListener {
            ...
            val team = findViewById<Switch>(R.id.switchTeam)
            val photo = findViewById<EditText>(R.id.editTextPhoto)
            val intent = Intent(this, MainActivity::class.java)
            val teamId = if (team.isChecked) 1 else 0
            db = Room.databaseBuilder(this, zakuDatabase::class.java, "zakuStore_db").build()
            lifecycleScope.launch {
                db.zakuDao().insert(zakuEntity(0,model.text.toString(), nick.text.toString(), teamId, photo.text.toString()))
            }
